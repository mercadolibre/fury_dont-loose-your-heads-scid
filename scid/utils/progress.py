...
class progress(object):
    ...
    def __init__(self, iterable, pi=1, desc='', logger=None, tot=None, dyn_pi=None, start_position=0):
        """
    ...
    def __iter__(self):
        self.display_text('{} pid {} | starting to iterate{}, updating every {} seconds'.format(
            self._now, self.pid, f' {self.desc}' if self.desc else '', self.pi
        ))
        self.i = 0
        last_pi = self.pi
    ...
    def _tot_aware_print(self):
        ...
        self.display_text(
            template.format(
                ...
                eta=(
                    f', eta {humanize_delta((self.tot - self.i) / max(self.speed, 1e-10))}'
                    # if self.i > 10 or time() - self.start > 300
                    # else ''
                ),
                custom_message='' if self.custom_msg_template is None else f', {self.custom_msg_template}'.format(
                    **self.custom_msg_params)
            )
        )
